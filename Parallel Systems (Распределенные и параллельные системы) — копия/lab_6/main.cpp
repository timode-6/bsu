...
constexpr long long size = 5000000;  
...
int main(){
  ...
  getExecutionTime("std::execution::seq", [workVec]() mutable{
      std::for_each(std::execution::seq, workVec.begin(), workVec.end(),
      [](double arg){return arg * arg * arg;});
  });
  
  getExecutionTime("std::execution::par", [workVec]() mutable{
      std::for_each(std::execution::par, workVec.begin(), workVec.end(), 
      [](double arg){return arg * arg * arg;});
  });
  
  getExecutionTime("std::execution::par_unseq", [workVec]() mutable{
      std::for_each(std::execution::par_unseq, workVec.begin(), workVec.end(),
      [](double arg){return arg * arg * arg;});
  });
